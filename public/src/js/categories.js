...
function endEdit(params, success, responseObj) {
	...
	if (success) {
		...
		article.style.backgoundColor = "#afefac";
		setTimeout(function () {
			article.style.backgoundColor = "white";
		}, 300);
...
function categoryDeleted(params, success, responseObj) {
	...
	if(success) {
		article.style.backgoundColor = "#ffc4be";
		setTimeout(function() {
			article.remove();
			location.reload();
		}, 300);
	...
	http.onreadystatechange = function() {
		if (http.readyState == XMLHttpRequest.DONE) {
			if (http.status == 200) {
				...
				alert("Category cuold not be save. Please try again");
